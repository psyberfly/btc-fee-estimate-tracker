...
const MovingAverageChartPage = ({ refreshTrigger }) => {
  ...
  //  const store = new LokiStore();
    ...
    async function handleDataFetch(since: Date): Promise<MovingAverage[] | Error> {
        try {
    ...
    const setData = async () => {
        try {
            ...
            const data = await handleDataFetch(requiredHistoryStartTime); //store.readMany(ChartType.movingAverage, requiredHistoryStartTime, requiredHistoryEndTime);
